...
import com.nfq.demo.entities.LoanEvent;
import com.nfq.demo.steps.FileCleanupTasklet;
...
public class BatchConfig {
    ...
    @Bean
    public JdbcBatchItemWriter<LoanEvent> itemWriter(DataSource dataSource) {
        return new JdbcBatchItemWriterBuilder<LoanEvent>()
    ...
    @Bean
    public FlatFileItemReader<LoanEvent> itemReader() {
        ...
        BeanWrapperFieldSetMapper<LoanEvent> fieldSetMapper = new BeanWrapperFieldSetMapper<>();
        fieldSetMapper.setTargetType(LoanEvent.class);
        ...
        DefaultLineMapper<LoanEvent> lineMapper = new DefaultLineMapper<>();
        lineMapper.setLineTokenizer(tokenizer);
        lineMapper.setFieldSetMapper(fieldSetMapper);

        return new FlatFileItemReaderBuilder<LoanEvent>()
    ...
    @Bean
    public Step importStep(FlatFileItemReader<LoanEvent> reader,
                           JdbcBatchItemWriter<LoanEvent> writer){
        return stepBuilderFactory.get("importStep")
                .<LoanEvent, LoanEvent>chunk(100)
                .reader(reader)
                .writer(writer)
                .build();
    ...
    @Bean
    public Job importUserJob(Step importStep, Step cleanupStep) {
